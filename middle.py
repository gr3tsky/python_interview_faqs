...
# weakref - это просто непрозрачный объект, который ссылается на все слабые ссылки на текущий объект.
# Это всеголишь деталь реализации, которая позволяет сборщику мусора информировать слабые ссылки о том, что 
# этот референт был собран, ибольше не допускать доступ к его основному указателю.

#######################################################################
#19. Для чего используется object.__slots__ ?
# Поведение по умолчанию можно изменить, задав __slots__ при определении класса. В __slots__ могут быть перечислены
# атрибуты для значений, которых требуется зарезервировать место (с точти зрения CPython в объекте каласса резервируется 
# место для массива указателей на Python-объекты)

#######################################################################
#20. Плюсы и минусы множественного наследования
# При правильном использовании можно очень гибко строить код, я использовал паттерн миксин и меня 
# вполне это устраивало. Минус в том, что такой вид наследования позволяет скрестить абсолютно несовместимые сущности,
# что приводит к багам. Также важно понимать MRO.

#######################################################################
#21. MRO (method resolutio order)
# Слева-направо, сверху-вниз

#######################################################################
#22. new-style и old-style классы
# Начиная с Python 2.2, помимо классических классов, существуют классы так называемого нового стиля.
# Классом нового стиля называется класс, у которого базовым классом является встроенный объкет (например
# list или dict) или объет object. Для классов старого и нового стиля отличаются результатом выполнения функции type(),
# а также вывод атрибутов __class__  и __bases__ для экземпляров классов.

#######################################################################
#23. Дескриптор протокол
# Если объект определяет сразу и __get__, и __set__, то он считается дескриптором данных (англ. data descriptor),
# Десрипторы, которые определили только __get__, называются дексрипторами не данных (англ. non-data descriptors),
# Их называт так, потому что они используются зля методов, но другие способы их применения также возможны.
